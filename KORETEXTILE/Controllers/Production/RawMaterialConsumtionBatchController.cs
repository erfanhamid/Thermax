...
{
    [ShowNotification]
    public class RawMaterialConsumtionBatchController : Controller
    {
        BEEContext context = new BEEContext();
        // GET: RawMaterialConsumtionBatch
        ...
        public ActionResult GetItemRemainQty(int itemId, int storeId, string date)
        {
            ...
            string sql = ("exec spGetRMSItemQty " + itemId + ", '" + DateTimeFormat.ConvertToDDMMYYYY(rmcdate) + "'");
            List<RMCItemQty> Data = context.Database.SqlQuery<RMCItemQty>(sql).ToList();
            var RemQty =Data[0];
            var qty = (decimal)RemQty.BalanceQty;
            return Json(new { RemQty = qty /*SaleCommonInfo.GetRemainingQtyForRMC(itemId)*/, Item = item , Rate = cost}, JsonRequestBehavior.AllowGet);
        }

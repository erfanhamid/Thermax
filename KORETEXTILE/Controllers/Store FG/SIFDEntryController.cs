...
{
    [ShowNotification]
    public class SIFDEntryController : Controller
    {
        BEEContext context = new BEEContext();
        // GET: SIID
        public ActionResult SIFDEntry()
        {
            var DepotID = context.BranchInformation.FirstOrDefault(x=> x.BrnachName == "Uttara").Slno;
            ViewBag.DepotId = DepotID;
            //ViewBag.Depot = LoadComboBox.LoadDepotByID(DepotID);
            ViewBag.Depot = LoadComboBox.LoadBranchInfo();
            ViewBag.StoreID = context.Store.FirstOrDefault(x => x.DepotID == DepotID && x.Name == "Factory FG").Id;
        ...
        public ActionResult SaveSIFD(SIFD SIID, List<SIFDLineItem> addedItems)
        {
            if (SIID != null && addedItems.Count > 0)
            {
                ...
                UserLog.SaveUserLog(ref context, new UserLog(SIID.SIFDNo.ToString(), SIID.Date, "SIFDFG", ScreenAction.Save));
        ...
        public ActionResult UpdateSIFD(SIFD SIID, List<SIFDLineItem> addedItems)
        {
            if (SIID != null && addedItems.Count > 0)
            {
                ...
                SIID.TotalCost = cost;
                context.SIFD.Add(SIID);
                UserLog.SaveUserLog(ref context, new UserLog(SIID.SIFDNo.ToString(), SIID.Date, "SIFDFG", ScreenAction.Update));
                InventoryTransactionBridge.InsertFromSIID(ref context, SIID, addedItems);
                context.SaveChanges();
                return Json(SIID.SIFDNo, JsonRequestBehavior.AllowGet);
            }
        ...
        public ActionResult DeleteSIFDByid(int id)
        {
            ...
            if (id != 0 && id.ToString() != "")
            {
                ...
                if (isExist != null)
                {
                    ...
                    UserLog.SaveUserLog(ref context, new UserLog(isExist.SIFDNo.ToString(), isExist.Date, "SIFDFG", ScreenAction.Delete));
